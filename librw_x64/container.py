...
class Function():
    ...
    def __str__(self):
        ...
        for instruction in self.cache:
            ...
            for iinstr in instruction.rbp_start:
            ...
            for iinstr in instruction.rbp_end:
                results.append("{}".format(iinstr))
...
class InstructionWrapper():
    def __init__(self, instruction):
        ...
        # 存放rax的表达式地址，用于set_ret_exp
        self.rax_exp_addr = None
        self.ret_mne = "q"

        # 用于处理j指令时，构建比较表达式
        self.param_addrs = dict()
    ...
    def instrument_rbp_start(self, iinstr, order=None):
        if order is not None:
            self.rbp_start.insert(order, iinstr)
        else:
            self.rbp_start.append(iinstr)

    def instrument_rbp_end(self, iinstr, order=None):
        if order is not None:
