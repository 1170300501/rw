...
class Instrument():
    ...
    def instrument_init_array(self):
        ...
        # 插入asan_init的汇编指令，生成init函数，以INIT_LOC作为起始地址（因为是额外添加的函数），添加到container中
    ...
    def get_mem_instrumentation(self, acsz, instruction, midx, free, is_leaf):
        ...
        enter_lbl = "%s_%x" % (sp.ASAN_MEM_ENTER, instruction.address)  # ASAN_ENTER标签
    ...
    def instrument_mem_accesses(self):
        for _, fn in self.rewriter.container.functions.items():
            ...
            for idx, instruction in enumerate(fn.cache):
                ...
                acsz = instruction.cs.operands[midx].size  # 原指令访问内存地址的操作数大小
                ...
                # 当前指令是否在free_registers中有未使用寄存器的列表
                ...
                # 调用get_mem_instrumentation得到插桩后的指令
    ...
    def instrument_stack(self):
        ...
        for addr, fn in self.rewriter.container.functions.items():
            ...
            for idx, instruction in enumerate(fn.cache[:20]):
                if instruction.op_str.startswith(sp.CANARY_CHECK):
                    need_red.append((addr, idx))  # 对每个函数，若存在使用canary（金丝雀）机制的栈溢出检查，则添加到需要插桩的列表中
                    break
            ...
            for idx, instruction in enumerate(fn.cache):
                if instruction.mnemonic.startswith("callq"):
                    if instruction.op_str.startswith("__longjmp"):
                        self.handle_longjmp(instruction)  # 对每个函数，存在longjmp则单独处理
        ...
        for addr, cidx in need_red:
            ...
            for idx, instruction in enumerate(fn.cache):
                if not is_poisoned:
                    if idx != cidx:  # 直到找到指定的第cidx条指令，CANARY_CHECK
                        continue
                    ...
                    pbase = store_exp
                    # 将mov的第二个操作数作为插桩的对象，即fs 寄存器 0x28 处的值
                    ...
                    if len(free_registers) > 0:
                        ...
                        need_save = False  # 当存在空寄存器，则无需保存到内存中
                    ...
                    # 在处理插桩后，找到验证栈溢出的指令，即，xorq %fs:0x28, mem_value，当异或结果为0，说明没有栈溢出
    ...
    def dump_stats(self):
        count = 0
        free_reg_sz = list()  # 用于记录每个memcheck中空闲的寄存器个数
        free_reg_cnt = defaultdict(lambda: 0)
        rflags_stats = [0, 0, 0, 0]  # 用于寄存器使用状态打印

        # memcheck_sites存放函数的初始地址和被插桩指令的idxs
        for addr, sites in self.memcheck_sites.items():
            ...
            for site in sites:
                ...
                if "rflags" in regs:
                    rflags_stats[0] += 1  # 0位表示rflags空闲的次数
                elif len(regs) == 0:
                    rflags_stats[1] += 1  # 1位表示所有寄存器都被用尽的次数
                elif "rax" in regs:
                    rflags_stats[2] += 1  # 2位表示rflags不空闲且rax空闲的次数
                elif len(regs) > 0:
                    rflags_stats[3] += 1  # 3位表示rflags不空闲且rax不空闲且存在其他空闲寄存器的次数
